...
- 短文：500-700 字（事实 200-300 字，观点 250-400 字）
- 长文：700-1000 字（事实 280-450 字，观点 350-600 字）
- 中文汉字计，标点不计
    ...
    async def generate(
        ...
        if article_type == "short":
            user_prompt = (
                ...
                f"写一篇爆款短评，500-700字。\n\n"
                f"硬性要求：\n"
                f"1. 前 1/3 到 1/2 篇幅讲新闻事实——把素材里的细节、数字、引语、背景挖出来讲，"
            ...
            user_prompt = (
                f"话题：{topic}{stance_hint}{context_block}\n\n"
                f"写一篇爆款深度评论，700-1000字。\n\n"
                f"硬性要求：\n"
                f"1. 前 1/3 到 1/2 篇幅讲新闻事实——把时间、地点、人物、过程、关键数字、关键引语都讲清楚，"
