...
发布流程:
  ...
  page.evaluate 调 /pcui/article/save(带 token header)保存草稿。
...
class BaijiahaoChannel(PlaywrightChannel):
    ...
    async def _do_publish(self, page, product: ArticleProduct, *, as_draft: bool) -> PublishResult:
        ...
        logger.error("百家号保存失败 errno=%s errmsg=%s", errno, errmsg)
        return PublishResult(
            channel=self.name, ok=False, status="save_failed",
            error=f"百家号保存失败 errno={errno} errmsg={errmsg}",
            raw=result,
        )
