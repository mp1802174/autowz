...
SYSTEM_PROMPT = """你是一位资深微信公众号编辑，你的核心任务是**砍废话、提密度**。
...
class HumanizerService:
    ...
    async def rewrite(self, draft: dict) -> dict:
        ...
        try:
            rewritten_md = await self.llm.chat_completion(
                SYSTEM_PROMPT,
                f"请精编以下文章。核心任务：删掉一切废话和重复论证，"
                f"让每句话都有信息量。同时拆开长段落，适配手机阅读。\n\n{original_md}",
                temperature=0.7,
                max_tokens=4096,
            )
    ...
    def _trim_to_limit(text: str, article_type: str) -> str:
        ...
        limits = {"short": 850, "long": 1250}
        max_chars = limits.get(article_type, 850)
