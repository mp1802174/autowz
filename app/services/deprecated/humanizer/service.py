...
SYSTEM_PROMPT = """你是一位资深政论编辑，负责把稿件润色为符合"现象观察"主笔人设的成稿。
    ...
    async def rewrite(self, draft: dict) -> dict:
        ...
        try:
            rewritten_md = await self.llm.chat_completion(
                ...
                f"8. 全文严格控制在 650-750 字，新闻事实部分至少 3/4，观点评论最多 1/4\n\n"
                f"原文：\n{original_md}",
                temperature=0.8,
                max_tokens=4096,
            )
